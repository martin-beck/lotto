...
bridge_context_(const context_t *origin, type_id type, type_id src_type,
                category_t cat)
{
    ASSERT(origin != NULL);
...
PS_SUBSCRIBE(CHAIN_INGRESS, EVENT_KEY_CREATE, {
    ...
    context_t ctx = bridge_context_(origin, EVENT_KEY_CREATE, cp->src_type,
                                    CAT_KEY_CREATE);
...
PS_SUBSCRIBE(CHAIN_INGRESS, EVENT_TASK_INIT, {
    ...
    context_t ctx                  = bridge_context_(origin, EVENT_TASK_INIT,
                                                    cp->src_type, CAT_TASK_INIT);
...
PS_SUBSCRIBE(CHAIN_INGRESS, EVENT_TASK_FINI, {
    ...
    context_t ctx                  = bridge_context_(origin, EVENT_TASK_FINI,
                                                    cp->src_type, CAT_TASK_FINI);
...
PS_SUBSCRIBE(CHAIN_INGRESS_BEFORE, EVENT_TASK_CREATE, {
    ...
    context_t ctx                    = bridge_context_(
        origin, EVENT_TASK_CREATE, cp->src_type, CAT_TASK_CREATE);
...
PS_SUBSCRIBE(CHAIN_INGRESS_AFTER, EVENT_TASK_CREATE, {
    const context_t *origin = (const context_t *)md;
    capture_point *cp       = (capture_point *)event;
    context_t ctx = bridge_context_(origin, EVENT_TASK_CREATE, cp->src_type,
                                    CAT_TASK_CREATE);
...
PS_SUBSCRIBE(CHAIN_INGRESS_BEFORE, EVENT_CALL, {
    const context_t *origin = (const context_t *)md;
    context_t ctx = bridge_context_(origin, EVENT_CALL, EVENT_CALL, CAT_CALL);
    (void)runtime_ingress_before(&ctx);
    return PS_OK;
})
...
PS_SUBSCRIBE(CHAIN_INGRESS, EVENT_TASK_DETACH, {
    const context_t *origin      = (const context_t *)md;
    capture_point *cp            = (capture_point *)event;
    capture_task_detach_event *dev = cp->task_detach;
    context_t ctx                = bridge_context_(origin, EVENT_TASK_DETACH,
                                                  cp->src_type, CAT_DETACH);
...
PS_SUBSCRIBE(CHAIN_INGRESS, EVENT_KEY_DELETE, {
    ...
    context_t ctx = bridge_context_(origin, EVENT_KEY_DELETE, cp->src_type,
                                    CAT_KEY_DELETE);
...
PS_SUBSCRIBE(CHAIN_INGRESS, EVENT_SET_SPECIFIC, {
    ...
    context_t ctx = bridge_context_(origin, EVENT_SET_SPECIFIC, cp->src_type,
                                    CAT_SET_SPECIFIC);
