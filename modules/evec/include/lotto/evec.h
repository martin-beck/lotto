...
void intercept_evec_prepare(void *addr) __attribute__((weak));
void intercept_evec_wait(void *addr) __attribute__((weak));
enum lotto_timed_wait_status
intercept_evec_timed_wait(void *addr, const struct timespec *restrict abstime)
    __attribute__((weak));
void intercept_evec_cancel(void *addr) __attribute__((weak));
void intercept_evec_wake(void *addr, uint32_t cnt) __attribute__((weak));
void intercept_evec_move(void *src, void *dst) __attribute__((weak));
...
lotto_evec_timed_wait(void *addr, const struct timespec *restrict abstime)
{
    if (intercept_evec_timed_wait != NULL) {
        return intercept_evec_timed_wait(addr, abstime);
    }
